...
# also, we set debconf keys to make APT a little quieter
        ...
        mkdir /usr/src/mysql; \
        cd /usr/src/mysql; \
        apt source mysql-community-server=${MYSQL_MINOR}; \
        cd mysql-community-${MYSQL_MINOR}; \
        ...
        debuild -b -uc -us; \
        MYSQL_DEB_VERSION=${MYSQL_VERSION}_${dpkgArch}; \
        cd ~; \
        dpkg --install \
                /usr/src/mysql/mysql-common_${MYSQL_DEB_VERSION}.deb \
                /usr/src/mysql/mysql-community-server_${MYSQL_DEB_VERSION}.deb \
                /usr/src/mysql/mysql-community-client_${MYSQL_DEB_VERSION}.deb \
                /usr/src/mysql/mysql-server_${MYSQL_DEB_VERSION}.deb \
                /usr/src/mysql/mysql-client_${MYSQL_DEB_VERSION}.deb; \
